...
def test_program_scope_enforced_scan(tmp_path, capsys):
    ...
    capsys.readouterr()
    rc = main([
        "--program", "bugrap:example-web3", "--store-dir", str(tmp_path),
        "-u", "https://api.example.org/x",
        "-u", "https://admin.example.org/x",
        "--dry-run", "--format", "json",
    ])
    out = capsys.readouterr().out
    assert rc == 0
...
def test_unknown_program_errors(tmp_path, capsys):
    rc = main(["--program", "bugrap:nope", "--store-dir", str(tmp_path),
               "-u", "https://x.example.org/"])
...
def test_fetch_program_cli(offline_dns, tmp_path, capsys):
    responses.add(responses.GET, "https://bugrap.io/robots.txt", status=200,
                  body="User-agent: *\nAllow: /")
    responses.add(responses.GET, "https://bugrap.io/bounties/demo", status=200,
                  body='<table><tr><td>*.demo.org</td></tr></table>')
    rc = main([
        "--fetch-program", "https://bugrap.io/bounties/demo",
        "--program", "demo", "--store-dir", str(tmp_path),
    ])
    out = capsys.readouterr().out
    assert rc == 0
